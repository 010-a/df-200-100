# simple_diffusion/scheduler/ddim.py (最终完整版 - 彻底禁用进度条)

import math
import numpy as np
import torch
# tqdm 不再需要，可以删除 from tqdm import tqdm
...
class DDIMScheduler:
    def __init__(
            self, num_train_timesteps=1000, beta_start=0.0001, beta_end=0.02,
            beta_schedule="cosine", clip_sample=True, set_alpha_to_one=True,
    ):
        self.num_train_timesteps, self.clip_sample = num_train_timesteps, clip_sample
        self.betas = cosine_beta_schedule(num_train_timesteps, beta_start, beta_end) if beta_schedule == "cosine" \
    ...
    def _step(self, model_output, timestep, sample, eta=1.0, generator=None):
        ...
        if self.clip_sample: pred_original_sample = torch.clamp(pred_original_sample, -1, 1)
    ...
    def add_noise(self, original_samples, noise, timesteps):
        ...
        return (sqrt_alpha_prod[:, None, None, None] * original_samples +
                sqrt_one_minus_alpha_prod[:, None, None, None] * noise)

    @torch.no_grad()
    def generate(
            self, model, batch_size=1, generator=None, eta=1.0,
            num_inference_steps=50, device=None, condition=None,
    ):
